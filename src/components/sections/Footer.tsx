"use client";

import { useI18n } from "@/lib/i18n";
import type { FooterData, BusinessInfo, SocialLinks } from "@/lib/types";

interface FooterProps {
  data: FooterData;
  business: BusinessInfo;
  social: SocialLinks;
}

export function Footer({ data, business, social }: FooterProps) {
  const { t } = useI18n();

  return (
    <footer className="bg-brand-950 text-white/80">
      <div className="container-narrow mx-auto px-5 sm:px-8 lg:px-16 py-12 lg:py-16">
        <div className="grid md:grid-cols-3 gap-10 items-start">
          {/* Brand */}
          <div>
            <h3 className="font-display text-xl font-semibold text-white mb-2">
              {business.name}
            </h3>
            <p className="text-sm text-white/50 leading-relaxed">
              {business.description}
            </p>
          </div>

          {/* Quick links */}
          <div className="flex gap-12">
            <div>
              <h4 className="text-xs font-semibold tracking-[0.15em] uppercase text-white/40 mb-3">
                {t.nav.services}
              </h4>
              <nav className="flex flex-col gap-2">
                <a href="#about" className="text-sm hover:text-white transition-colors">
                  {t.nav.about}
                </a>
                <a href="#services" className="text-sm hover:text-white transition-colors">
                  {t.nav.services}
                </a>
                <a href="#reviews" className="text-sm hover:text-white transition-colors">
                  {t.nav.reviews}
                </a>
                <a href="#contact" className="text-sm hover:text-white transition-colors">
                  {t.nav.contact}
                </a>
              </nav>
            </div>
          </div>

          {/* Legal + social */}
          <div className="md:text-right">
            <div className="flex md:justify-end gap-3 mb-4">
              {social.instagram && (
                <a
                  href={social.instagram}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
                  aria-label="Instagram"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                </a>
              )}
              {social.facebook && (
                <a
                  href={social.facebook}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
                  aria-label="Facebook"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
              )}
            </div>
            <div className="flex md:justify-end gap-4 text-xs text-white/40">
              <a href={data.privacyPolicyUrl} className="hover:text-white/60 transition-colors">
                {t.footer.privacy}
              </a>
              <a href={data.termsUrl} className="hover:text-white/60 transition-colors">
                {t.footer.terms}
              </a>
            </div>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="mt-10 pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-white/30">
          <p>{data.copyright}</p>
          <p>
            {t.footer.builtBy}{" "}
            <a
              href={data.builtBy.url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-white/50 hover:text-white/70 transition-colors"
            >
              {data.builtBy.name}
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
}
